...
class TextProcessor():
    ...
    def tokenize(self, text):
        ...
        tokens = filter(
            lambda token: token.lower() not in STOP_WORDS and len(token) > 2 and not
            (bool(re.compile('\d').search(token))),
            tokens)
        tokens = map(lambda token: wordnet_lemmatizer.lemmatize(token), tokens)
        tokens = map(lambda token: ''.join(character for character in token if character.isalnum()), tokens)
        return tokens
    ...
    def nouns(self, tokens):
        return [token for token, tag in self.tagger.tag(tokens) if tag in self.NOUN_POS_TAGS]

    def get_named_entity_count(self, text):
        tokens = self._no_stop_tokens(text)
        pos_tagged_sentence = self.pos_tag(tokens)
        named_entity_tree = nltk.ne_chunk(pos_tagged_sentence)
        if not named_entity_tree:
            return 0
        named_entity_count = reduce(lambda value1, value2: value1 + value2,
                                    map(lambda node: 1 if isinstance(node, nltk.tree.Tree) else 0, named_entity_tree))
        return named_entity_count

    def _no_stop_tokens(self, text):
        tokens = []
